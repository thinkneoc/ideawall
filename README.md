...
所以, 我要做一个跨平台的, 完美的, 桌面层软件工具. 就自己用, 这时间也花得值. 
...
#### 技术债务
以下技术债务, 仅在用户量达到某一数量才会考虑开发.
1.桌面商店: 暂时没有写, 这个, 没人用的话, 写了也白写. 主要功能是 HTML 发售平台. 
2.桌面源项目用户点评模块: 同上. 主要为增强用户粘性的产物. 
3.目前桌面层无法接收鼠标事件, 这个, 难搞哦~ 后期可能会处理, 为了安全性考虑, 也可能不会处理. 

#### 开源声明
此项目仅供学习参考, 禁止未经作者许可擅自商业运营和各类证书申请, 违者必究. 
