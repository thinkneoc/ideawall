...
function createOne(display, paramJson) {
    ...
    // xwindow.webContents.on('did-finish-load', () => {
    //     xwindow.webContents.send('ipc_window_wall_init', display.id);//给这个窗口发送一个初始化完成指令, 并传递设备信息过去.
    // });
...
function creat(displayId, paramJson) {
    logger.info("[Process][MainProcessHelper][WallWindow]初始化壁纸窗口组");
    //拿一下主屏幕宽高, 可能要用到
    AppVar.setAppVar({
    ...
    for (let i in displays) {
        ...
        wallwindows.push(xwindow);
        _wallwindows[display.id] = {display: display, window: xwindow};
    }
    AppVar.setAppVar({
        _wallwindows: _wallwindows,
    });
    return wallwindows;
}
