...
function creat(deskId, paramJson) {
    logger.info("[Process][MainProcessHelper][PreviewWindow]初始化桌面预览窗口");
    ...
    xwindow.loadURL(url.format({
        pathname: path.join(__dirname, relative + "./view/components/Preview.html"),
        protocol: 'file:',
        slashes: true
    }) + "?windowKey=_previewwindow&windowId=" + xwindow.id + "&deskId=" + deskId + lang.json2UrlParams(paramJson));
    ...
    xwindow.on('closed', function () {
        logger.info("[Process][MainProcessHelper][_PreviewWindow_.on._closed_]桌面预览窗口关闭");
