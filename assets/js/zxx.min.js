...
var ZXX = function () {
    ...
    this.censusScript = function () {
        if ($('html').is('[zxx-census]') && window.navigator.onLine) {
            T.i('http://tajs.qq.com/stats?sId=66445930', 'body');
        }
        return this;
    }
                ...
                if ((rs.origin + '/').indexOf(vm.origin) > -1) {
                    ...
                    switch (rs.data.command) {
                        ...
                        case 'control.showloading':
                            try {
                                proxy._controlwindow.webContents.send('ipc_render_control_showloading', data.text, data.dur);
                            } catch (e) {
                            }
                            break;
                        case 'control.showmsg':
                            try {
                                proxy._controlwindow.webContents.send('ipc_render_control_showmsg', data.content, data.type, data.duration, data.closable, data.nodestory);
                            } catch (e) {
                            }
                            break;
                        default:
