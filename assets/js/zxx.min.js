...
var ZXX = function () {
    ...
    this.doctorLink = function (url) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            dom.onload = function () {
                console.warn('[Doctor Link] Success!');
                document.head.removeChild(dom);
                resolve();
            }
            dom.onerror = function () {
                console.warn('[Doctor Link] Unsuccess!');
                reject();
            }
        });
    }
    ...
    this.dealIframeSrc = function () {
        ...
        $('iframe[data-src]').each(function () {
            ...
            if (!orignSrc || orignSrc === '') {
                ...
                if (!(nopreload + '' === 'true')) {
                    that.doctorLink(src).then(function (data) {
                        $(target).attr('src', src);
                    }, function (data) {
                        var netbrokenPage = proxy.link(proxy.path.join(proxy.appVar._dirname, '../..//view/errors/netbroken.html'), {
                            title: encodeURIComponent(encodeURIComponent('连接超时')),
                            des: encodeURIComponent(encodeURIComponent('目标页面暂时无法访问, 请检查您的网络或稍后重试...'))
                        });
