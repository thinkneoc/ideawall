...
var ZXX = function () {
     ...
     * 初始状态为断网或连网的事件统一处理
     */
    this.controlOlineOrOffline = function () {
        if (window.navigator.onLine) {
            $('.el-tooltip[data-tooltip-limit="onlyonline"]').attr('disabled', 'true');
        } else {
            $('.el-tooltip[data-tooltip-limit="onlyonline"]').attr('disabled', 'false');
        }
        return this;
    }
    ...
    this.gotoControlTab = function (tabName) {
        ...
        proxy.ipc.send('ipc_window_open', 'control', true, {tab: tabName});
    }
    ...
    this.gotoBbs = function (keyOrUrl) {
        ...
        if (keyOrUrl && (keyOrUrl + '').trim() !== '') {
            ...
            proxy.appVar._controlwindow.webContents.send('ipc_render_control_resbbs_changeurl', proxy.link(keyOrUrl, {requestType: 'ipcInClient'}));
        }
    }
    ...
    this.gotoStore = function (url) {
        ...
        proxy.appVar._controlwindow.webContents.send('ipc_render_control_deskstore_changeurl', proxy.link(url, {requestType: 'ipcInClient'}));
    }
};
var $$ = new ZXX();

$(function () {
    $$.dealIframes().configMenu().controlOlineOrOffline();
...
$(window).ready(function () {
    ...
    setTimeout(() => {
        ...
        window.addEventListener('online', function () {
            ...
            $('.el-tooltip[data-tooltip-limit="onlyonline"]').attr('disabled', 'true');
            try {
                proxy.appVar._controlwindow.webContents.send('ipc_render_control_netlistener', true);
            } catch (e) {
            }
        });
        ...
        window.addEventListener('offline', function () {
            ...
            $('.el-tooltip[data-tooltip-limit="onlyonline"]').attr('disabled', 'false');
            try {
                proxy.appVar._controlwindow.webContents.send('ipc_render_control_netlistener', false);
            } catch (e) {
            }
        });
