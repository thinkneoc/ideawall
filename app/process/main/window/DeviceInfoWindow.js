...
function creat(displayId, paramJson) {
    logger.info("[Process][MainProcessHelper][DeviceInfoWindow]初始化桌面预览窗口");
    let xy = calcPosition();
    // 创建浏览器窗口。
    xwindow = new Electron.BrowserWindow({
        ...
        maxWidth: xy.size.width,
        maxHeight: xy.size.height,
        resizable: false,
        movable: true,
        minimizable: true,
        maximizable: false,
    ...
    xwindow.loadURL(url.format({
        pathname: path.join(__dirname, relative + "./view/components/DeviceInfo.html"),
        protocol: 'file:',
        slashes: true
    }) + "?windowKey=_deviceinfowindow&windowId=" + xwindow.id + "&displayId=" + displayId + lang.json2UrlParams(paramJson));
    ...
    xwindow.on('closed', function () {
        logger.info("[Process][MainProcessHelper][_DeviceInfoWindow_.on._closed_]桌面预览窗口关闭");
...
function calcPosition() {
    ...
    let size = {width: 320, height: 480};
    let xy = {x: (cp[0] - size.width - 1), y: cp[1]};
    xy.x = (xy.x <= 0 ? undefined : xy.x);//可能超出视宽
