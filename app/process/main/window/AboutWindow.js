...
function creat(paramJson) {
    logger.info("[Process][MainProcessHelper][AboutwWindow]初始化桌面预览窗口");
    ...
    xwindow.loadURL(url.format({
        ...
        protocol: 'file:',
        slashes: true
    }) + "?windowKey=__aboutwindow&windowId=" + xwindow.id + lang.json2UrlParams(paramJson));
    ...
    xwindow.on('closed', function () {
        logger.info("[Process][MainProcessHelper][_AboutwWindow.on._closed_]桌面预览窗口关闭");
