    ...
    this.dealIframe = function (target, src) {
        if (!$(target).attr('loadingHandler')) {
            this.doctorLink(src).then(function (data) {
                ...
                var fairPage = '';
                if ($(target).attr('data-backpage')) {
                    fairPage = proxy.link(proxy.path.join(proxy.appVar._dirname, '../../view/' + $(target).attr('data-backpage')));
                } else {
                    ...
                    fairPage = proxy.link(proxy.path.join(proxy.appVar._dirname, '../../view/errors/netbroken.html'), {
                        title: encodeURIComponent(encodeURIComponent(nb_title)),
                        des: encodeURIComponent(encodeURIComponent(nb_des))
                    });
                }
                console.warn('[Loading No Success] Auto Turn To => ' + fairPage);
                $(target).attr('src', fairPage);
                try {
                    if ($(target).attr('data-cmd')) {
                        top.vm.doctorReport($(target).attr('data-cmd'), $(target).attr('name'), fairPage);
                        vm.doctorReport($(target).attr('data-cmd'), $(target).attr('name'), fairPage);
                    }
                } catch (e) {
                    //... 尝试通知应用程序
                }
